    ...
    if (mutation.type === 'childList') {
      ...
      for (let i = 0; i < addedNodes.length; i++) {
        ...
        if (node.classList && node.classList.contains('close-button')) {
          console.log("change2")
          // The element with the desired class was added to the DOM,
          // so you can add the event listener here
          node.addEventListener('click', function() {
            console.log('Element with desired class was clicked');
          });
        }
      }
    }
...
card.forEach(box => {
  box.addEventListener('click', ()=> {
    element = document.querySelector(".grid");
        ...
        element.querySelector("#close-popup").appendChild(button);
        });  
  })
});
