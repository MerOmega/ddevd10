...
card.forEach(box => {
  box.addEventListener('click', (box)=> {
    element = box.querySelector(".relevant-movies").classList.replace("hiden","uncover");
    if (element.classList.contains('A')) {
  // If it does, remove A and add B
  element.classList.remove('A');
  element.classList.add('B');
} else if (element.classList.contains('B')) {
  // If it doesn't, it must have the class B, so remove B and add A
